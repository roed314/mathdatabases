...
One of the main roles that a mathematical database plays is to record the results of difficult computations.
...
Some databases are embedded within computer algebra systems such as Sagemath, Magma, or GAP \citelist{\cite{magmadb} \cite{smallgroups} \cite{transgroups}}, which allows a user to easily perform further computations with the objects in the database.
...
Searching requires a more substantial infrastructure than is needed for just displaying data, and one of our aims for this article is to provide advice for mathematicians who might like to create searchable databases.
...
Projects also have various ways of reaching out to potential developers, ranging from academic workshops and conferences, to the standard open source tools such as Github \cite{github}, to solicitations for data \citelist{\cite{findstat} \cite{hog} \cite{oeis}}.
...
\section{Nuts and bolts} \label{sec:nutsbolts}
