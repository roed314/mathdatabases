...
\subhead{Website}
    ...
    The LMFDB follows such an approach, where most content is generated on the fly through a dedicated web application based on Flask~\cite{flask} and HTML templates, and only a small amount of the content is human-curated.
...
Even though we supported such searches through our web interface, many such searches took more than a couple of minutes to perform within MongoDB, which caused our web front-end time out.
In PostgreSQL, most of these queries finish in a couple of seconds, both when the number of results is not large enough to justify using an index (the first three rows of Table~\ref{table:ramps}), and when GIN indexes apply (the last row of Table~\ref{table:ramps}).
\begin{table}[h!]
  \begin{center}
...
Once these queries have been constructed, we need to execute the search and pass the results to the Jinja \cite{jinja} template that actually crafts the webpage seen by the user.
