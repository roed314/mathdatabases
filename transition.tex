...
Including some that predated the project\todo{add references}.
\todoinline{R1: the discussion of other approaches does not reference
prior/on-going efforts by people involved in the LMFDB project.  The
Cremona database of elliptic curves over Q, the Jones-Roberts local field
database, the number field tables hosted by the pari project, the
Jones-Roberts number field database.  It is possible the authors are
intentionally avoiding a discussion of these means of distributing number
theoretic data, but they give examples which could be mentioned here.}
...
\subsection{Back End}
\todoinline{R2: (3) I found section 3.1 a little weird in the choice of categories}
...
\subhead{Human readable files}
    ...
    \todoinline{R2: Didn't mention one of the more serious issues with print: the sheer volume
of paper it would take to print out the LMFDB is beyond impractical. }
\todoinline{R2: Sure csv counts as “human readable” but so does JSON. I think this category really seems to be about data without any sort of front-end for which you have to actually look through the data manually to find anything, i.e. data that is “meant to be read by humans” more so than the format being readable by humans. For small data sets, that is possible.}
...
databases used by computer algebra systems are not large.  But magma has
some optional databases which are quite large (
http://magma.maths.usyd.edu.au/magma/download/db/ lists a couple which go
over a gigabyte)}
    \todoinline{R2: (application dependent). I also disagree with some of the discussion here. Just because it is in an application specific format does not mean that it is not effi- ciently searchable. It depends entirely on how well the custom data format was implemented. The real issue here seems to be a lack of compatibility and long term maintainability.}

\subhead{Database structure}
\todoinline{R2: (database structure) “Database” and “optimized for searching” are not synony- mous to me. Yes, indexes are a natural part of a database format, but if the index is not indexing the “right’ things then the searches are still terrible. If fact, isn’t the whole issue with switching from MongoDB to PostgreSQL was that MongoDB was not optimized for the searches you were trying to perform. I think the key here is longterm stability and compatibility and the many existing tools already developed both for searching and administrating.}
...
\todoinline{R2: “direct access” to me really sounds like the “no front end” case. You are simply
...
\subhead{Computer algebra system}
    ...
    This option is extremely convenient for the user of those software systems, however, this is not optimal for very large databases, as these would increase the size of the computer algebra system in question.
    \todo{mention optional databases}

\subhead{Website}
\todoinline{R2: Seems like the website section should be split into static versus dynamic. There are many examples of each type in the world and they have different pros/cons and require quite different requirements from the developer to set-up.}
...
\subsection{Hosting}
\todoinline{R2:  Seems like this section could be split up into front-end hosting versus database hosting. Surprisingly there was no discussion of the MongoDB hosting or AWS hosting pros/cons, or any of other major hosting resources/services.
(a) It seems like the authors are confusing hosting the data with hosting the front-end.\\
(b) Personal hard-drive: The accessibility of this data depends on the accessibility of \\the front end, not where the data is actually stored. Missed a major con of the front end utilizing the resources of your personal machine and exposing it to security risks.                                                                         \\
(c) Personal webpage is only a host if it is static web-page with all the data written  \\
out in the webpage (i.e., no database behind it)                                        \\
(d) web framework: is this for storing the database or the front-end?                   \\
}
\todoinline{not every combination is valid}
\todoinline{Add note saying that front-end and back-end do not need to be hosted in the same device}
...
Over the last year\todo{R2: specify the year}, we have transitioned from MongoDB to PostgreSQL, a popular open source implementation of the SQL language that grew out of earlier projects founded in the 1970s and 80s.
...
\todoinline{R2: Why did you write your own and not use any of the existing migration tools?}
\todoinline{many workarounds around MongoDB limitations in its early stages imputed limitations on using an existing tool to automatically migrate the data between MongoDB and PostgresSQL}
...
\todoinline{R2:  By choosing “database” format you automatically have indexes. You also seem to say the Mon- goDB and PostgreSQL use the same type of binary tree based indexes, but do not mention why PostgreSQL is so much faster. If I were someone writing my own data- base it seems like you are saying to use PostgreSQL if at all possible (i.e., if I don’t really need a huge number of different document types). Again table of pros/cons would be helpful here.}
\todoinline{mention that MongoDB and PostgreSQL are optimised for two different data sets}
...
\todoinline{R1: The queries should be explained, that the
first is for number fields (then it is clear), and that the second is for
elliptic curves over Q, which is otherwise not explained.}
\begin{table}[h!]
  \begin{center}
...
\todoinline{R2: So what if they cache? Isn’t this something that is configurable by the admin? If you are going to talk about caching, then I would be interested in: How long is the cache stored? Is the cache stored across users? What are the pros/cons of different caching settings?}
...
Additionally, PostgreSQL offers numerous open source front ends and administration tools~\cite{pgadmin,adminer} that lower the learning curve for the SQL language.\todo{R2: MongoDB has numerous open source admin tools as well. Why are the PostgreSQL tools better?}
...
Adapting the LMFDB codebase to use PostgreSQL took longer, and we are still finishing up a few bits of that part of the transition.\todo{Still finishing up as of when? This really needs a date as that comment will quickly become out of date in a publication.}
...
\todoinline{R2: Is your PostgreSQL query interface being made available as a stand-alone tool or only within the LMFDB?}
\todoinline{Mention that we plan to make it a stand-alone tool, and it already helped with other projects}
...
\todoinline{R1: 
this cites the new mechanism for generating statistics pages and
gives the impression that it is widely used, but I don't think that is the
case.  Many areas do not have statistics pages, and some have it but do not
...
\todoinline{R1: this is on an improved version of "random object", and may
give the impression that the new version is used universally, but I don't
think many parts of the LMFDB have this feature\\
Edgar: This has been addressed}
...
\todoinline{R1: 
I am not sure that there are internal verifications done that
defining polynomials of number fields are irreducible.  The second example
in the sentence on the trace of a modular form is unclear.  There are many
types of modular forms, and different ways a number field might be
associated to one (needed to interpret "inert prime").  I can infer what
they mean, but not all readers would be able to.\\
  The point is that one can write small programs to check for
consistency in the data and that PostgreSQL makes this easier.  But, I
don't think they justify why this is easier with PostgreSQL.  If one knows
how the data is stored for say elliptic curves over Q and for weight 2
newforms defined over Q, I think writing a program to check for consistency
would be easy with either type of database.  My point is that if doing this
is much easier with PostgreSQL, it needs to be explained. \\
