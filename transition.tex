...
\todoinline{R2:  By choosing ``database'' format you automatically have indexes. You also seem to say the MongoDB and PostgreSQL use the same type of binary tree based indexes, but do not mention why PostgreSQL is so much faster. If I were someone writing my own database it seems like you are saying to use PostgreSQL if at all possible (i.e., if I don't really need a huge number of different document types). Again table of pros/cons would be helpful here.}
...
\todoinline{R1: this cites the new mechanism for generating statistics pages and gives the impression that it is widely used, but I don't think that is the case.  Many areas do not have statistics pages, and some have it but do not use the new approach }
