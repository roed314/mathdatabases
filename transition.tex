...
For many ages the only option available was to use a print format, for example in a book or an article, and rely on a publisher to distribute its content and to make it available to the rest of math community.
Once a publisher found, this solution was practical and simple, however given how uncommon typos were, this is far from an ideal solution.
\todo{mention hard to fix errors}
Nowadays, a type of digital medium is preferred and distribution relies mostly on the internet.
This raises three coupled questions regarding the choices for back end, front end, and hosting.
In other words,
\begin{itemize}
...
\end{itemize}
we will overview some of the possible answers for each question.

\subsection{Back End}

We start by providing some options in how to encode the database in a machine readable format.
We will sort the options base on easy is to search on content.
...
\begin{itemize}
  \item human readable files
    ...
    However, it might be cumbersome and impractical to search or manipulate the data without software aid.
    \todo{add examples}
  \item application dependent files
    ---
    A more evolved solution is to store the data set in application dependent format.
    In many cases such a choice enables direct manipulation of the desired objects.
    For example if one use SAGE pickles, a generic data format that allows to save and load generic SAGE objects.
    However, the dataset most likely will not be stored in an efficient way to search by content, and thus must content searches would need to loop over every item on the dataset, what we call a sequential search.
    \todo{mention The GAP Small Groups Library}
  \item database structure
    ---
    For last one may store the dataset in database search format, i.e., in a format optimized to search the data by content.
    There are many ways to achieve this, in this article we fill focus on two types:
    \begin{itemize}
      \item relational database --- a collection of ledger-style tables with a fixed schema, where the structure in row of such table is constant througout.
      ...
      \item document oriented database --- a schema free database, where each object is stored in a single instance in the database, and every stored object can be different from every other.
        One of such instantiations is MongoDB.
    \end{itemize}
    in Section blah we will go over some of the differences between PostgreSQL and MongoDB.
