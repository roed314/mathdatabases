...
\begin{itemize}
  ...
  \item website
    ...
    These pages can be humanly curate each object page \citelist{\cite{cantorsattic}, \cite{fanosearch}} or, automatically generate them as static web pages \citelist{\cite{etc}, \cite{graphclasses}, \cite{SyzygyData}, \cite{knotinfo}}.
    Alternatively, one can instead use an web application to generate each page when it is accessed.
    ...
    Other examples of mathematical databases using such approach contain \citelist{\cite{knotatlas}, \cite{hog}}.
\end{itemize}
...
\end{center}
\end{table}
\todoinline{mention that both queries in Table 2 become instantaneous on the second run}
\todoinline{cite Table 2}
...
\end{tabular}
\caption{Times for fetching the rows/objects satisfying the given constraint.}
