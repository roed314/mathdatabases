...
We focus on one particular case study: the effort to migrate the L-functions and modular forms database (LMFDB)~\cite{lmfdb} from MongoDB~\cite{mongo} to PostgreSQL~\cite{postgres}.
...
One of the main roles that a mathematical database plays is to record the results of difficult\todo{and demanding?} computations.
...
Others are available online in various formats; we will discuss various methods of data distribution in Section~\ref{sec:nutsbolts}.

Collecting a large number of examples into one database makes possible a valuable paradigm for experimental mathematics: the ability to search through examples for a few that have particular desired properties.
We discuss searching within the context of the LMFDB at length in Section \ref{sec:MvsP}.
In addition to the LMFDB, a number of other mathematical databases also have search functionality as one of their core features \citelist{\cite{umrk} \cite{findstat} \cite{oeis} \cite{isc} \cite{cydata}}.
Searching requires a more substantial infrastructure than is needed for just displaying data, and one of our aims for this article is to provide advice\todo{better wording} for mathematicians who might like to create searchable databases.
...
Projects also have various ways of reaching out to potential developers, ranging from academic workshops and conferences, to the standard open source tools such as Github \cite{github}, to solicitations\todo{Edgar: solicitation sounds weird} for data \citelist{\cite{findstat} \cite{hog} \cite{oeis}}.
...
The LMFDB hosts a variety of databases and connects them through the Langlands program, we recommend~\cite{cremona-16} to learn more about it.\todo{maybe also mention just simple browsing}
...
For example, one can search and access data in relation to other data in the database. \todo{This seems redundant and awkwardly phrased}
...
Moreover, we expect to further speed up such queries by taking advantage of GIN indexes in PostgreSQL, which are designed to efficiently handle such searches.
\begin{table}[h!]
  \begin{center}
  \begin{tabular}{l|r|r}
Query & MongoDB\footnote{We used WiredTiger for comparisons since it outperforms MMAP when using normal amounts of RAM} & PostgreSQL\\
\hline
...
\end{center}
\end{table}
\todoinline{Add 2, 3, 73 to the table, since this uses an index; add column for whether an index is used}
\todoinline{Footnote 1 is missing}
\todoinline{mention that both queries become instantaneous on the second run}

% One of the issues with the MongoDB implementation is that we were counting the number of results before anything else
...
\todo{second time we mention this}
...
\todoinline{Discuss the fact that we can now return a list of results without returning the total count: this had major speed consequences}
