...
We focus on one particular case study: the effort to migrate the L-functions and modular forms database (LMFDB) from MongoDB to PostgreSQL.
...
There are many valuable resources focusing on theorems and propositions, from general encyclopedias such as Wikipedia and Mathworld to the preprint server arXiv to domain specific references such as Groupprops. \todo{add citations}
...
\todoinline{We plan to move the next section on mathematical databases here, giving brief summaries both to clarify what kinds of projects we actually are discussing, and as a way to introduce these projects to a reader who may find them interesting.}

\todoinline{Explain our intended audience: mathematicians wanting to create databases, anyone who might be considering switching from MongoDB to PostgreSQL, mathematicians interested in large software projects who might be interested in some of the lessons we've learned from the LMFDB.}
...
\item Prime pages \url{https://primes.utm.edu/}
\end{itemize}
...
Until the last several decades, the only option available was to use a print format, for example in a book or an article, and rely on a publisher to distribute its content and to make it available to the rest of mathematical community.
Since the publisher takes care of most of the technical issues, this method offers the benefit of simplicity to the author, but distribution in print offers a number of disadvantages compared to digital distribution via the internet.
First, any mistakes are difficult to fix, and adding new features or data is a long process.  \todo{mention hard to fix errors}
Second, print distribution is less immediately accessible than online distribution, putting barriers in the way of the database user.
But since the digital distribution of mathematical data is not taken on by publishers, authors have many more choices to make.
...
\begin{itemize}
  \item human readable files
    ---
     Storing the data set in a human readable format, like a  plain text file or a comma-separated values file, is one of the simplest possible solutions.
  ...
  \item application dependent files
    ---
    A more complicated solution is to store the dataset in an application dependent format.
    ...
    \todo{mention The GAP Small Groups Library}
  \item database structure
    ...
    \begin{itemize}
      \item relational database --- a collection of ledger-style tables with a fixed schema, where the structure in each row of is constant.
        These kind of databases have dominated the industry for decades \todo{who am I to say this?} and some of their famous instantiations are Oracle, MySQL, PostgreSQL, and SQLite.
      \item document oriented database --- a schema free database, where each object is stored in a single instance in the database, and every stored object can be different from every other.
        The most popular example is MongoDB.
    \end{itemize}
    In Section \ref{sec:MvsP} we will contrast PostgreSQL and MongoDB.
    \todo{mention additional software might be required for users to to manipulate the data?}
\end{itemize}
...
\begin{itemize}
  \item personal hard-drive --- no initial set up cost, but the  
  \item personal webpage --- On a personal webpage (easy for many academics), but limits scope (aren't running your own webserver)
